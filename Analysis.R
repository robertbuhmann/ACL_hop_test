...
#hop test
hop <- data[, c("ID", "hop.inj", "hop.opp")]
hop <- hop %>%
  pivot_longer(cols = (2:3),
               names_to = "Limb",
               values_to = "hop")
hop <- hop %>%
  mutate(Limb = (ifelse(Limb == "hop.inj", 1, 2)))%>%
  mutate(Limb = factor(Limb),
         ID = factor(ID))
...
descrip <- merge(ext_60, ext_300, by = c("Limb", "ID"))%>%
  ...
  merge(hop, by = c("Limb", "ID"))

means <- aggregate(cbind(ext_60, ext_300, flex_60, flex_300, flex_ecc, hop) ~ Limb, 
                    data = descrip, FUN = mean)

sd <- aggregate(cbind(ext_60, ext_300, flex_60, flex_300, flex_ecc, hop) ~ Limb, 
                data = descrip, FUN = sd)
...
plot4 <- ggplot(data = data, aes(x = ecc.ham.inj, y = hop.inj)) +
  geom_point(shape = 21, color = "black", fill = 'lightcoral', size = 3)+
  theme(text = element_text(family = "serif"),
        panel.background = element_blank(),
        axis.line = element_line(colour = "black"),
